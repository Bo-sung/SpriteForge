...
public sealed unsafe class OffscreenRenderer : IDisposable
{
        ...
        Bounds bounds, float yaw, float pitch, RenderOptions opts)
    {
        ...
        // Roll (bank) about the view axis: rotate the world-up vector around the camera forward direction.
        Vector3 up = Vector3.UnitY;
        float rollRad = opts.CamRoll * MathF.PI / 180f;
        if (MathF.Abs(rollRad) > 1e-5f)
        {
            Vector3 forward = Vector3.Normalize(target - eye);
            up = Vector3.Transform(Vector3.UnitY, Quaternion.CreateFromAxisAngle(forward, rollRad));
        }
